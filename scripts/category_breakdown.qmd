...
##remove nas
cat_breakdown_df_clean <- cat_breakdown_df[!is.na(cat_breakdown_df$primary_policy_category_broad), ]
...
cat_breakdown_bar_df <- cat_breakdown_df_clean %>% 
  group_by(primary_policy_category_broad, primary_subcategory) %>% 
  summarise(count=n()) 
...
category_breakdown <- ggplot() +
  geom_bar(data=cat_breakdown_bar_df, aes(x=primary_policy_category_broad, y=count, fill=primary_subcategory), stat="identity", position="dodge") +
