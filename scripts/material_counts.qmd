...
material_list <- c("aluminum", "asphalt", "brick", "cement", "concrete", "concrete reinforcing steel", "glass", "wood", "gypsum board", "mineral wood", "insulation", "iron", "rebar", "steel", "timber")
...
sums_df_tidy <- sums_df_nontidy %>%
  ...
  filter(!name %in% c("rebar", "concrete reinforcing steel", "cement"))
...
policy_jurisdiction <- policy_database_materials %>%
  select(legal_status, other:ncol(policy_database_materials)) %>%
  select(-other)



jursidiction_df_nontidy <- policy_jurisdiction %>%
  summarise_at(c(2:ncol(policy_jurisdiction)), sum, na.rm = TRUE) %>% pivot_longer(., cols=everything(), values_to="n_policies")

## we now need to remove some duplicates which will require the use of human eyes and brain. 

## we need to find the uses of "wood" which really actually only are wood

wood_pol_jur <- policy_jurisdiction %>% 
  filter(wood>0 & `mineral wood` >0) %>% 
  select(`mineral wood`, wood, covered_material_s)
##above is the amount that contain both "mineral wood" as well as "wood" -- read through them and save into the following the number that contain both but it's JUST because the word "wood" is included in "mineral wood"

wood_to_remove_jur <- 4 ## I have saved 4 (all of them)


##concrete reinforcing steel will count as "concrete" but it shouldn't!
crs_concrete_jur <- policy_jurisdiction %>% 
  filter(`concrete reinforcing steel` > 0 & concrete > 0) %>% 
  select(`concrete reinforcing steel`, concrete, covered_material_s)


crs_to_remove_jur <- 4 ## I have saved 4 (all of them) (this is unsurpising)

##these above have all been the same 4 policies causing an issue (@ california)

#rebar and steel will be the same so we're going to make sure there are no double counts

rebar_steel_jur <- policy_jurisdiction %>% 
  filter(rebar > 0 & steel > 0) %>% 
  select(rebar, steel, covered_material_s)

rebar_steel_to_remove_jur <- 3 ##3 contain both "rebar" AND "steel"


concrete_cement_jur <- policy_jurisdiction %>% 
  filter(concrete > 0 & cement > 0) %>% 
  select(concrete, cement, covered_material_s)

cement_to_remove_jur <- 4

timber_to_add_jur <- sum(policy_jurisdiction$timber, na.rm = TRUE)

jursidiction_df_tidy <- jursidiction_df_nontidy %>%
  mutate(subtract = case_when(
    name == "wood" ~ wood_to_remove_jur, ## removing "mineral wood board insulation" from "wood"
    name == "concrete" ~ crs_to_remove_jur, ##removing concrete reinforcing steel from "concrete"
    name == "rebar" ~ rebar_steel_to_remove_jur, ##removing rebar where it ALSO mentions steel separately,
    name == "cement" ~ cement_to_remove_jur,
  )) %>% 
    mutate(add = ifelse(
      name == "wood",timber_to_add,0)) %>% 
  mutate(subtract = ifelse(is.na(subtract), 0, subtract)) %>%
